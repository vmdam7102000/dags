# dags/crypto/aggregate_crypto_ohlcv_4h_xmr_backdate_dag.py
...
logger = logging.getLogger("aggregate_crypto_ohlcv_4h_xmr_backdate_dag")
...
def aggregate_window(conn, start_ts_ms: int, end_ts_ms: int, symbol: str) -> int:
    sql = f"""
    WITH base AS (
        SELECT
            symbol,
...
with DAG(
    dag_id="aggregate_crypto_ohlcv_4h_xmr_backdate_dag",
    description="Backdate aggregate 3m OHLCV candles into 4h timeframe for XMR only",
    default_args={
        "owner": "crypto-data",
    ...
    def aggregate() -> Dict[str, int]:
        ...
        try:
            ...
            while window_start < end_ts_ms:
                ...
                inserted = aggregate_window(conn, window_start, window_end, SYMBOL)
                total_inserted += inserted
                logger.info(
                    "Aggregated %s rows for %s window %s - %s",
                    inserted,
                    SYMBOL,
            ...
            logger.info(
                "Total aggregated rows: %s for %s range %s - %s",
                total_inserted,
                SYMBOL,
