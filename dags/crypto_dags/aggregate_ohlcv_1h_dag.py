# dags/crypto/aggregate_crypto_ohlcv_1h_dag_dag.py
...
CONFIG = load_yaml_config("crypto_configs/ohlcv_1h.yml")["ohlcv_1h"]
DB_CFG = CONFIG["db"]
LOOKBACK_DAYS: int = int(DB_CFG.get("lookback_days", 7))
...
BUCKET_MINUTES = int(DB_CFG.get("bucket_minutes", 60))
BUCKET_MS = BUCKET_MINUTES * 60 * 1000
SCHEDULE = CONFIG.get("schedule", "5 * * * *")

logger = logging.getLogger("aggregate_crypto_ohlcv_1h_dag")
...
with DAG(
    dag_id="aggregate_crypto_ohlcv_1h_dag",
    description="Aggregate 3m OHLCV candles into 1h timeframe",
    default_args={
        "owner": "crypto-data",
