# dags/crypto/aggregate_crypto_ohlcv_15m_dag_dag.py
...
BUCKET_MINUTES = int(DB_CFG.get("bucket_minutes", 15))
BUCKET_MS = BUCKET_MINUTES * 60 * 1000
SCHEDULE = CONFIG.get("schedule", "30 * * * *")

logger = logging.getLogger("aggregate_crypto_ohlcv_15m_dag")
...
def _bucket_start_ts(ts_ms: int) -> int:
    """
    Floor timestamp (ms) to the start of the bucket.
    """
...
with DAG(
    dag_id="aggregate_crypto_ohlcv_15m_dag",
    description="Aggregate 3m OHLCV candles into 15m timeframe",
    default_args={
        "owner": "crypto-data",
