# dags/crypto/aggregate_crypto_ohlcv_1d_dag_dag.py
...
CONFIG = load_yaml_config("crypto_configs/ohlcv_1d.yml")["ohlcv_1d"]
DB_CFG = CONFIG["db"]
LOOKBACK_DAYS: int = int(DB_CFG.get("lookback_days", 90))
BATCH_DAYS: int = max(1, int(DB_CFG.get("batch_days", 30)))
...
BUCKET_MINUTES = int(DB_CFG.get("bucket_minutes", 1440))
BUCKET_MS = BUCKET_MINUTES * 60 * 1000
SCHEDULE = CONFIG.get("schedule", "20 2 * * *")

logger = logging.getLogger("aggregate_crypto_ohlcv_1d_dag")
...
with DAG(
    dag_id="aggregate_crypto_ohlcv_1d_dag",
    description="Aggregate 3m OHLCV candles into 1d timeframe",
    default_args={
        "owner": "crypto-data",
